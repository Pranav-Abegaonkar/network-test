...
const getNetworkStatistics = async () => {
  try {
    ...
    const networkStats = await window.VideoSDK.getNetworkStats(options);
    const downloadSpeed = networkStats["downloadSpeed"];
    const uploadSpeed = networkStats["uploadSpeed"];

    console.log('Getting the Netwokr stats:', {
      downloadSpeed, uploadSpeed
    })

    textDiv.textContent = `d:${downloadSpeed} u:${uploadSpeed}`
  } catch (e) {
...
function initializeMeeting() {
  ...
  //   meetingId: "h5zh-xhy5-ql8s", // required
